...
- 유의확률(p-value)의 계산 : 귀무가설 H0가 사실일 때, 검정통계량 X의 분포분산 X에서 x0(=표본 자료로부터 계산된 검정통계량의 값)보다 대립가설 방향으로 더 극단적인 값이 나올 확률
    - 유의수준 100a %의 검정법 : 자료로부터 계산된 유의확률(p-value)이 주어진 유의수준 a보다 작은 경우에 귀무가설 H0를 기각함\
    p-value <= a면 H0를 기각

#### Bartlett 검정과 Levene 검정
- k(>= 2)개의 모집단에 대한 등분산 검정
- 가설
    - H0 : 시그마1^2 = 시그마2^2 = ... = 시그마k^2
    - H1 : 적어도 하나 이상의 시그마i^2은 나머지와 다르다.
- Bartlett 검정
    - 모집단이 모두 정규분포를 따르는 경우 적용함
    - k=2인 경우 등분산 F검정, k > 3인 경우는 Baralett의 검정
- Levene 검정
...
- 범주형 자료 분석
    ...
    - 독립성 검정 : 서로 다른 두 범주형 변수간에 연관성이 있는지를 검정

- 카이제곱 독립성 검정 : 2개의 범주형 변수를 요약하는 r * c의 2차원 분할표
    - 두 변수가 독립인 경우에 예상되는 각 셀 별 기대빈도 도출
    - 각 셀 별 빈도와 기대빈도의 차이의 크기를 이용하여 검정
    - 차이가 작으면 두 변수는 독립
    - 차이가 크면 두 변수는 독립이 아님
![Alt text](/reference_statistics/image-91.png)

- 가설
    - 귀무가설 H0 : 두 범주형 변수는 서로 독립(관계가 없다)
    - 대립가설 H1 : 두 범주형 변수는 독립적인 관계가 아니다(관계가 있다)
- 검정통계량과 표본분포
    - 귀무가설(H0)이 사실인 경우
    - 표본의 수가 충분히 큰 경우 (모두 기대빈도 5이상)
![Alt text](/reference_statistics/image-92.png)

- 유의확률의 계산 : 귀무가설 H0이 사실일 때, 검정통계량 X의 표본분포에서 x0(= 표본자료로부터 계산된 검정통계량의 값)보다 더 큰 값이 나올 확률
    - 유의수준 100a %의 검정법 : 자료로부터 계산된 유의확률(p-value)이 주어진 유의수준 a보다 작은 경우에 귀무가설 H0를 기각함\
    p-value <= a면 H0를 기각


## 20. 시계열 데이터의 특징, 시계열 분해법, 자기상관
- 시계열 자료 : 시간의 흐름에 따라 순차적으로 관찰된 자료들의 집합\
예시)
    - 월별 소비자 물가지수, 일별 주가지수
    - 일일 강수량, 일일기온, 연간 지진의 발생수
    - 월별 상품판매량, 월별 상품재고량, 일별 상품매출액
    - 월별 교통사고 건수, 월별 범죄 발생수

#### 탐색적 시계열 분석
- 이동평균평활법 : 가장 최근의 m-기간동안의 자료들의 단순 평균을 다음 기간의 예측값으로 추정\
Zn을 시점 n에서 실제값, Zn+1을 시점 n에서 추정한 시점 n+1의 예측값이라고 하면,\
Zn+1 = 1/m * (Zn + Zn-1 + ... + Zn-m+1)

- 지수평활법 : 이동평균평활법과 같이 과거의 관측값을 활용하여 다음 기간의 값을 예측하지만, 최근의 자료에 더 많은 가중치를 부여하는 방법\
Zn을 시점 n에서의 실제값, Sn을 시점 n에서 지수평활예측값(Zn+1)이라고 하고, 평활 상수 a는 0~1 사이의 값이라고 할 때,\
![Alt text](/reference_statistics/image-93.png)

- 분해법(추세요인, 계절요인)
    - 변동요인의 분해과정
        - 원계열 Zt에 추세선(예. 단순선형회귀)을 적합하여 추세성 Tt을 추정\
        Zt -> Tt = a + Bt
        - 원계열 Zt과 추세성 Zt의 비율 St을 정의\
        St = Zt / Tt
        - k번째 계절에 대한 계절지수 SIk는 해당 계절의 St들의 평균으로 구함
    - 예측
        - T시점의 예측값 Zt = (a + BT)SIk (SIk : T시점이 K번째 계절이라고 할 때의 계절지수)
...
- 정상 시계열 모형
    ...
    - 비정상 시계열 : 정상성 조건으로 하나라도 만족하지 못하는 시계열 자료\
    비정상 시계열은 정상성을 만족하는 정상 시계열 자료로 만든 후 자기회귀(AR), 이동평균(MA), 자기회귀이동평균(ARMA) 모형을 적합하는 시계열 분석을 수행함
    - 자기회귀(AR), 이동평균(MA) 모형의 식별을 위해 자기상관 및 부분자기상관함수의 형태를 이용
        - 자기상관계수(AC) : 일정한 간격으로 떨어진 두 시점에서의 시계열 자료 간의 선형적 연관성
        - 부분자기상관계수(PAC) : 일정한 간격으로 떨어진 두 시점 사이의 상관관계 분석 시, 중간 시점의 영향을 제외하고 측정하는 선형적 연관성